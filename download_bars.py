...
class DownloadApp(EClient, wrapper.EWrapper):
    ...
    def perform_write_on_db(self, contract: Contract,
        ...
        df['primaryExchange'] = contract.primaryExchange
        df['secType'] = contract.secType

        size = self.args.size.replace(' ', "_").lower()
        df.to_sql(f"ib_hist_data_{size}")

    def save_data(self, contract: Contract, bars: BarDataList,
                  write_on_db: int) -> None:
        data = [[
        ...
        if write_on_db == 1:
            return self.perform_write_on_db(contract, df)
    ...
    def headTimestamp(self, reqId: int, headTimestamp: str) -> None:
        ...
        if ts > self.args.end_date:
            logging.warning("Data for %s is not available before %s", contract,
                            ts)
            self.done = True
            return
        ...
        if self.daily_files():
            ...
            self.current = self.current.replace(hour=0,
                                                minute=0,
                                                second=0,
                                                microsecond=0)
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str) -> None:
        ...
        if len(self.pending_ends) == 0:
            ...
            for rid, bars in self.bar_data.items():
                self.save_data(self.requests[rid], bars, self.args.write_on_db)
    ...
    def start(self):
        ...
        self.started = True
        for contract in self.contracts:
            self.reqHeadTimeStamp(self.next_request_id(contract), contract,
                                  self.args.data_type, 0, 1)

    @iswrapper
    def error(self, req_id: TickerId, error_code: int, error: str):
        super().error(req_id, error_code, error)
        if req_id < 0:
            logging.debug("Error. Id: %s Code %s Msg: %s", req_id, error_code,
                          error)
        else:
            logging.error("Error. Id: %s Code %s Msg: %s", req_id, error_code,
                          error)
...
def _validate_in(value: str, name: str, valid: List[str]) -> None:
    if value not in valid:
        raise ValidationException(
            f"{value} not a valid {name} unit: {','.join(valid)}")
...
def _validate(value: str, name: str, valid: List[str]) -> None:
    ...
    if len(tokens) != 2:
        raise ValidationException(
            "{name} should be in the form <digit> <{name}>")
...
def main():
    ...
    argp.add_argument("--symbols-file",
                      type=str,
                      help="file to get symbols list")
    argp.add_argument("--write-on-db", type=int, default=0, help="write on DB")
    argp.add_argument(
        "-p",
        "--port",
        type=int,
        default=4001,  # 7496,
        help="local port for TWS connection")
    argp.add_argument("--size", type=str, default="1 min", help="bar size")
    argp.add_argument("--duration",
                      type=str,
                      default="1 D",
                      help="bar duration")
    argp.add_argument("-t",
    ...
    argp.add_argument("--currency",
                      type=str,
                      default="USD",
                      help="currency for symbols")
    argp.add_argument("--exchange",
                      type=str,
                      default="SMART",
                      help="exchange for symbols")
    argp.add_argument(
        "--primary-exchange",  # <- use to avoid contract ambiguity
        type=str,
        default="ISLAND",
        help="primary exchange for symbols")
    argp.add_argument("--localsymbol",
                      type=str,
                      default="",
                      help="local symbol (for futures)")
    argp.add_argument("--security-type",
                      type=str,
                      default="STK",
    ...
    def make_simple_contract(s: str):
        make_contract(s, args.security_type, args.currency, args.exchange,
                      args.primary_exchange, args.localsymbol)

    logging.debug(f"args={args}")
    contracts = []
    for s in args.symbol:
        # contract = make_simple_contract(s)
        contract = make_contract(s, args.security_type, args.currency,
                                 args.exchange, args.primary_exchange,
                                 args.localsymbol)
